...
describe('AssetsGaView Frontend Logic & Permission Tests', () => {
  ...
  test('AssetsGaView enforces assets_ga write permission strictly on CRUD buttons and mutations', async () => {
    ...
    assert.match(source, /canWriteAssets\s*=\s*computed\(\s*\(\)\s*=>\s*hasWritePermission\('assets_ga'\)\s*\)/)
    assert.doesNotMatch(source, /hasWritePermission\('assets_ga'\)\s*\|\|\s*hasWritePermission\('assets'\)/)
