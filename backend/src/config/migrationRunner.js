...
const TRANSACTION_CONTROL_PATTERN =
  /^\s*(?:BEGIN\s*(?:;|\b(?:WORK|TRANSACTION)\b)|COMMIT|ROLLBACK|SAVEPOINT|RELEASE)\b/im;
...
function requireExactEnvironment(environment, name, expected) {
  ...
  if (!actual || actual !== expected) {
    throw new Error(
      `${name} wajib sama persis dengan target terkonfigurasi: ${expected}`,
    );
  }
}
...
export async function loadVersionedMigrations(
  ...
  for (const name of names) {
    ...
    if (TRANSACTION_CONTROL_PATTERN.test(executable)) {
      throw new Error(
        `Migration tidak boleh mengatur transaction sendiri: ${name}`,
      );
    }
    if (PSQL_META_COMMAND_PATTERN.test(executable)) {
      throw new Error(
        `Migration tidak boleh berisi psql meta-command: ${name}`,
      );
    }
...
export function assertMigrationAuthorization({
  ...
  if (environment.ALLOW_DB_MIGRATIONS !== "true") {
    throw new Error(
      "Apply migration ditolak: ALLOW_DB_MIGRATIONS=true belum diberikan.",
    );
  }

  const recoveryProofId = normalizedText(
    environment.MIGRATION_RECOVERY_PROOF_ID,
  );
  if (!/^[a-zA-Z0-9][a-zA-Z0-9._/:@-]{7,159}$/.test(recoveryProofId)) {
    throw new Error(
      "MIGRATION_RECOVERY_PROOF_ID wajib menunjuk bukti restore yang terverifikasi.",
    );
  }
  ...
  if (
    ...
    !/^[a-zA-Z0-9][a-zA-Z0-9._/:@-]{2,159}$/.test(changeId)
  ) {
...
export function assertDatabaseIdentity(
  identity,
  {
    expectedDatabase,
    apply = false,
    requireNonSuperuser = process.env.REQUIRE_NON_SUPERUSER === "true",
  },
) {
  ...
  if (requireNonSuperuser && identity.is_superuser === true) {
    throw new Error(
      "Migration harus memakai role owner non-superuser yang terdedikasi.",
    );
  }
  if (
    apply &&
    (identity.is_replica === true || identity.transaction_read_only === true)
  ) {
    throw new Error(
      "Target database read-only/replica tidak dapat menerima migration.",
    );
  }
}
...
export function buildMigrationPlan(migrations, appliedMigrations) {
  ...
  for (const applied of appliedMigrations) {
    ...
    if (local.name !== applied.name || local.checksum !== applied.checksum) {
      throw new Error(
        `Checksum migration yang sudah diterapkan berubah: ${applied.name}`,
      );
    }
...
async function acquireMigrationLock(client) {
  ...
  if (result.rows[0]?.acquired !== true) {
    throw new Error(
      "Migration lain sedang berjalan; advisory lock tidak tersedia.",
    );
  }
}
