...
} from "../src/config/migrationRunner.js";
...
test("production migration apply requires a change ID and exact target confirmation", () => {
  ...
  assert.equal(
    assertMigrationAuthorization({ databaseConfig, environment, apply: true })
      .changeId,
    "change-123",
  );
