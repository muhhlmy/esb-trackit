...
<template>
  <AppModal :is-open="isOpen" title="Export Pengiriman" :subtitle="`Unduh ${shipments.length} data yang sedang tampil`" icon="download" size="sm" @close="emit('close')">
    <div class="space-y-4">
      ...
      <div class="flex flex-col-reverse gap-2 border-t border-slate-200 pt-4 sm:flex-row sm:justify-end"><button type="button" @click="emit('close')" class="min-h-10 rounded-xl border border-slate-200 px-4 text-xs font-bold text-slate-600 hover:bg-slate-50">Batal</button><button type="button" :disabled="!shipments.length" @click="exportData" class="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl bg-[#0A51B0] px-5 text-xs font-bold text-white hover:bg-[#08458f] disabled:cursor-not-allowed disabled:opacity-50"><span aria-hidden="true" class="material-symbols-outlined text-[17px]">download</span>Unduh XLSX</button></div>
    </div>
  </AppModal>
</template>
