  ...
  >
    <div class="space-y-4">
      <div class="flex items-start gap-3 rounded-2xl border border-[#CFE0F8] bg-[#F4F8FF] p-4">
        ...
        <div>
          ...
          <p class="mt-1 text-xs leading-relaxed text-slate-500">
            Export berisi data Pengiriman yang sedang tampil setelah filter diterapkan.
          </p>
        </div>
      </div>
      <div class="flex items-center justify-between rounded-xl border border-slate-200 px-3 py-2.5">
        ...
        ><span class="text-sm font-bold text-[#0A51B0]">{{ shipments.length }} pengiriman</span>
      </div>
      <p
        v-if="!shipments.length"
        class="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-semibold text-amber-700"
      >
