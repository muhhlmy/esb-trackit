...
<template>
  <div ref="mainScope" class="w-full max-w-7xl mx-auto space-y-4 sm:space-y-6 font-sans">
    ...
    <!-- Header Card -->
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3.5 sm:gap-4 bg-white dark:bg-slate-900 border border-[#E5EAEF] dark:border-slate-800 p-4 sm:p-6 rounded-2xl shadow-sm gsap-admin-el">
      <div class="space-y-1 sm:space-y-1.5 w-full sm:w-auto">
        ...
        <h1 class="text-xl sm:text-3xl font-extrabold text-[#0F172A] dark:text-white tracking-tight flex items-center gap-2">
          <span>Knowledge Base</span>
          <span class="text-[10.5px] sm:text-xs px-2 sm:px-2.5 py-0.5 rounded-full font-bold bg-[#ECF2FF] dark:bg-indigo-950/80 text-[#5D87FF] dark:text-indigo-300 border border-[#5D87FF]/20">
            Admin CMS
          </span>
        </h1>
    ...
    <div class="gsap-admin-el">
      ...
      >
        <div class="flex items-center gap-2.5 sm:gap-3 min-w-0">
          <div class="w-8 h-8 sm:w-9 sm:h-9 rounded-lg bg-[#ECF2FF] dark:bg-slate-800 text-[#5D87FF] dark:text-indigo-300 flex items-center justify-center group-hover:bg-[#5D87FF] group-hover:text-white transition-colors shrink-0">
            <LayoutGrid class="w-4 h-4" />
          </div>
          <div class="min-w-0">
            <div class="text-xs sm:text-sm font-semibold text-[#0F172A] dark:text-white truncate">Kategori Knowledge Base</div>
            <div class="text-[11px] sm:text-xs text-[#64748B] dark:text-slate-400 font-normal truncate">
              Kelola topic cards yang tampil di Browse Topics Help Center.
            </div>
          </div>
        </div>
        <ChevronRight class="w-4 h-4 text-[#94A3B8] group-hover:text-[#5D87FF] group-hover:translate-x-0.5 transition-all shrink-0 ml-2" />
      </RouterLink>
    </div>
    ...
    <div class="grid grid-cols-3 gap-2 sm:gap-4 gsap-admin-el">
      ...
      <div class="p-3 sm:p-5 rounded-xl bg-white dark:bg-slate-900 border border-[#E2E8F0] dark:border-slate-800 transition-colors hover:border-slate-300 dark:hover:border-slate-700">
        <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 mb-1.5 sm:mb-3">
          <div class="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 flex items-center justify-center shrink-0">
            <FileText class="w-3.5 h-3.5 sm:w-4 sm:h-4" />
          </div>
          <span class="text-[10px] sm:text-xs font-semibold text-[#64748B] dark:text-slate-400 truncate">Total Panduan</span>
        </div>
        <p class="text-lg sm:text-2xl font-bold text-[#1E293B] dark:text-white tracking-tight tabular-nums">
          {{ stats.total }}
        </p>
      </div>
      ...
      <div class="p-3 sm:p-5 rounded-xl bg-white dark:bg-slate-900 border border-[#E2E8F0] dark:border-slate-800 transition-colors hover:border-slate-300 dark:hover:border-slate-700">
        <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 mb-1.5 sm:mb-3">
          <div class="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 flex items-center justify-center shrink-0">
            <CheckCircle class="w-3.5 h-3.5 sm:w-4 sm:h-4" />
          </div>
          <span class="text-[10px] sm:text-xs font-semibold text-[#64748B] dark:text-slate-400 truncate">Published</span>
        </div>
        <p class="text-lg sm:text-2xl font-bold text-[#1E293B] dark:text-white tracking-tight tabular-nums">
          {{ stats.published }}
        </p>
      ...
      <div class="p-3 sm:p-5 rounded-xl bg-white dark:bg-slate-900 border border-[#E2E8F0] dark:border-slate-800 transition-colors hover:border-slate-300 dark:hover:border-slate-700">
        <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 mb-1.5 sm:mb-3">
          <div class="w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-violet-50 dark:bg-violet-950/60 text-violet-600 dark:text-violet-400 flex items-center justify-center shrink-0">
            <Sparkles class="w-3.5 h-3.5 sm:w-4 sm:h-4" />
          </div>
          <span class="text-[10px] sm:text-xs font-semibold text-[#64748B] dark:text-slate-400 truncate">Custom</span>
        </div>
        <p class="text-lg sm:text-2xl font-bold text-[#1E293B] dark:text-white tracking-tight tabular-nums">
          {{ stats.custom }}
        </p>
    ...
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2.5 sm:gap-3 gsap-admin-el">
      ...
      <div class="flex flex-wrap items-center gap-2">
        ...
        <div class="flex items-center p-0.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-xs shrink-0 overflow-x-auto no-scrollbar">
          <button
            v-for="st in [
              { key: 'all', label: 'Semua' },
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl border border-[#E2E8F0] dark:border-slate-800 overflow-hidden gsap-admin-el shadow-sm">
      ...
      <div class="md:hidden divide-y divide-[#F1F5F9] dark:divide-slate-800/60">
        ...
        >
          ...
          <div class="flex items-center justify-between gap-2">
            <div class="flex items-center gap-1.5 flex-wrap min-w-0">
              ...
              <span
                ...
                :class="c.status === 'DRAFT' ? 'bg-amber-50 dark:bg-amber-950/40 text-amber-600 dark:text-amber-400 border border-amber-200/50' : 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400 border border-emerald-200/50'"
              >
                <span
          ...
          <div class="cursor-pointer" @click="editDoc(c.id)">
            <h3 class="text-[13px] font-bold text-[#1E293B] dark:text-slate-100 hover:text-[#2563EB] transition-colors leading-snug">
              {{ c.title }}
            </h3>
            <p class="text-xs text-[#64748B] dark:text-slate-400 line-clamp-2 mt-1 font-normal leading-relaxed">
              {{ c.summary }}
            </p>
          </div>

          <!-- Card Footer: Severity & Tags -->
          <div class="flex items-center justify-between gap-2 pt-1.5 border-t border-[#F1F5F9] dark:border-slate-800/60 text-[11px]">
            <!-- Severity -->
    ...
    <Transition name="fade">
      ...
      >
        <div class="bg-white dark:bg-slate-900 border border-[#E2E8F0] dark:border-slate-800 rounded-2xl p-5 sm:p-6 max-w-sm w-full shadow-xl space-y-4">
          ...
          <div>
            <h3 class="text-sm sm:text-base font-bold text-[#1E293B] dark:text-white">Hapus dokumen?</h3>
